...
import * as functions from "firebase-functions/v1"; // Keep v1 for Auth triggers (it's stable)
...
const RAID_COST = 20;
const RAID_SUCCESS_RATE = 0.75;
const ADMIN_EMAIL = "iznoatwork@gmail.com";

// ============================================================================
// 1. AUTH TRIGGER: AUTOMATIC PROFILE CREATION
// ============================================================================
// We use v1 here because it's simpler for background triggers
...
// ============================================================================
// 2. GAMEPLAY: EXECUTE RAID (PvP) - UPDATED TO V2
// ============================================================================
export const executeRaid = onCall(async (request) => {
  // v2 Syntax: 'auth' and 'data' are properties of 'request'
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "IDENTIFY YOURSELF");
  }
  ...
  return db.runTransaction(async (t) => {
    ...
    if (!userDoc.exists || !targetDoc.exists) {
      throw new HttpsError("not-found", "TARGET LOST");
    }
    ...
    if ((userData.energy || 0) < RAID_COST) {
      throw new HttpsError("resource-exhausted", "ENERGY DEPLETED");
    }
    ...
    if (isSuccess) {
      ...
      stolenAmount = Math.min(stolenAmount, 500);
      stolenAmount = Math.max(stolenAmount, 10);
...
// ============================================================================
// 3. GAMEPLAY: CLAIM MISSION BOUNTY - UPDATED TO V2
// ============================================================================
export const claimMission = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "NO AUTH");
  }

  const { energyCost, reward } = request.data;
  ...
  return db.runTransaction(async (t) => {
    ...
    if ((userData.energy || 0) < energyCost) {
      throw new HttpsError("resource-exhausted", "LOW ENERGY");
    }
...
// ============================================================================
// 4. ADMIN TOOL: SEED THE WORLD - UPDATED TO V2
// ============================================================================
export const seedWorld = onCall(async (request) => {
  if (request.auth?.token.email !== ADMIN_EMAIL) {
    throw new HttpsError("permission-denied", "OVERSEER CLEARANCE REQUIRED");
  }
