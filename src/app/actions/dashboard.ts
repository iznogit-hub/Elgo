...
const getGitHubStats = unstable_cache(
  ...
  ["dashboard-github"],
  { revalidate: 3600, tags: ["dashboard", "coding"] },
);

const getCodeStats = unstable_cache(
  async () => {
...
const getValorantStats = unstable_cache(
  async () => {
    ...
    try {
      ...
      const mmrData = mmrRes.ok
        ? (await mmrRes.json()).data?.current_data
        : null;
  ...
  ["dashboard-valorant"],
  { revalidate: 600, tags: ["dashboard", "gaming"] },
);

const getLoLStats = unstable_cache(
  async () => {
...
const getSystemStats = unstable_cache(
  async () => {
    ...
    try {
      ...
      const cpuPercent = Math.min(
        Math.round((finalLoad / cpuCores) * 100),
        100,
      );
  ...
  ["dashboard-system"],
  { revalidate: 60, tags: ["dashboard", "system"] },
);
