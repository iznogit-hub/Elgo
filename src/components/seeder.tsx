  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      Object.entries(NICHE_DATA).forEach(([id, data]) => {
        ...
        const ref = doc(db, "sectors", id); // using 'sectors' to keep it clean
        
        // 3. Prepare the payload (Clean up JSX icons if needed, Firestore can't store JSX)
        // We strip the 'icon' field because React Components can't go to DB.
        // We will map icons back on the frontend based on the ID.
  ...
  return (
    <div className="p-6 border border-white/10 bg-white/5 rounded-sm space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
           <h3 className="text-sm font-black font-orbitron text-white uppercase">Database_Seeder_V1</h3>
           <p className="text-[10px] font-mono text-gray-400">Push local `NICHE_DATA` to Firestore.</p>
        </div>
      </div>

      <div className="p-4 bg-black/50 border border-white/10 font-mono text-[9px] text-gray-400">
      ...
      <Button 
        ...
        className="w-full bg-cyan-900/20 border border-cyan-500/50 text-cyan-400 hover:bg-cyan-500 hover:text-black font-black uppercase text-xs h-10"
      >
